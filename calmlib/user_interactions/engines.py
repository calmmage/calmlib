...
class UserInteractionEngine(ABC):
    ...
    @abstractmethod
    def ask_user(self, question: str, **kwargs) -> Optional[str]:
        """Ask user a text question and get string response"""
        pass
    
    @abstractmethod 
    def ask_user_choice(self, question: str, choices: Union[List[str], Dict[str, str]], **kwargs) -> Optional[str]:
        """Ask user to choose from options"""
        pass
    
    @abstractmethod
    def ask_user_confirmation(self, question: str, **kwargs) -> Optional[bool]:
        """Ask user yes/no question and get boolean response"""
        pass
    
    @abstractmethod
    def ask_user_raw(self, question: str, **kwargs) -> Optional[Any]:
...
class BotspotEngine(UserInteractionEngine):
    ...
    def __init__(self, chat_id: Optional[int] = None, state=None):
        try:
            from botspot.user_interactions import (
                ask_user as botspot_ask_user,
    ...
    def _ensure_async_context(self):
        if self.chat_id is None or self.state is None:
            raise ValueError("BotspotEngine requires chat_id and state to be set")
    
    async def _ask_user_async(self, question: str, **kwargs) -> Optional[str]:
        self._ensure_async_context()
        return await self.botspot_ask_user(self.chat_id, question, self.state, **kwargs)
    
    async def _ask_user_choice_async(self, question: str, choices: Union[List[str], Dict[str, str]], **kwargs) -> Optional[str]:
        self._ensure_async_context()
        return await self.botspot_ask_user_choice(self.chat_id, question, choices, self.state, **kwargs)
    
    async def _ask_user_confirmation_async(self, question: str, **kwargs) -> Optional[bool]:
        self._ensure_async_context()
        return await self.botspot_ask_user_confirmation(self.chat_id, question, self.state, **kwargs)
    
    async def _ask_user_raw_async(self, question: str, **kwargs) -> Optional[Any]:
        self._ensure_async_context()
        return await self.botspot_ask_user_raw(self.chat_id, question, self.state, **kwargs)
    
    # Sync wrappers - these will need to be handled differently in async context
    def ask_user(self, question: str, **kwargs) -> Optional[str]:
        import asyncio
        try:
            return asyncio.run(self._ask_user_async(question, **kwargs))
        except RuntimeError:
            # Already in async context - caller needs to use async methods directly
            raise RuntimeError("BotspotEngine requires async context - use async methods directly")
    
    def ask_user_choice(self, question: str, choices: Union[List[str], Dict[str, str]], **kwargs) -> Optional[str]:
        import asyncio
        try:
            return asyncio.run(self._ask_user_choice_async(question, choices, **kwargs))
        except RuntimeError:
            raise RuntimeError("BotspotEngine requires async context - use async methods directly")
    ...
    def ask_user_confirmation(self, question: str, **kwargs) -> Optional[bool]:
        import asyncio
        try:
            return asyncio.run(self._ask_user_confirmation_async(question, **kwargs))
        except RuntimeError:
            raise RuntimeError("BotspotEngine requires async context - use async methods directly")
    ...
    def ask_user_raw(self, question: str, **kwargs) -> Optional[Any]:
        import asyncio
        try:
            return asyncio.run(self._ask_user_raw_async(question, **kwargs))
        except RuntimeError:
            raise RuntimeError("BotspotEngine requires async context - use async methods directly")
...
class ServiceTelegramBotEngine(UserInteractionEngine):
    ...
    def __init__(
        self, chat_id: Optional[int] = None, use_dev: bool = False, timeout: int = 300
    ):
        """
    ...
    async def _send_and_wait(self, text: str, reply_markup=None) -> Optional[str]:
