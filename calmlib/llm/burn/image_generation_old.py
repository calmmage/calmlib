...
async def generate_image(
    prompt: str,
    model: str | None = None,  # Auto-selects if None
    # Canonical kwargs
    n: int | None = 1,
    size: str | None = None,  # "1024x1024", "1792x1024", etc.
    quality: str | None = None,  # "standard", "high", "hd"
    response_format: str | None = "b64_json",  # "url" or "b64_json"
    style: str | None = None,  # "natural" or "vivid" (OpenAI-specific)
    # Additional kwargs
    **kwargs: Any,
) -> Any:
    """
    Unified async image generation function supporting multiple providers.

    Args:
        prompt: Text description of the image to generate
        model: Model name or None to auto-select based on available API keys
        n: Number of images to generate (default: 1)
        size: Image dimensions (provider-specific defaults apply)
        quality: Image quality setting (provider-specific)
        response_format: Return format - "url" or "b64_json" (default: b64_json)
        style: Style hint (OpenAI-specific: "natural" or "vivid")
        **kwargs: Additional provider-specific parameters

    Returns:
        LiteLLM ImageResponse object
    """
    logger.debug(f"Starting image generation with prompt: '{prompt[:50]}...'")
    logger.debug(f"Parameters: model={model}, n={n}, size={size}, quality={quality}")

    # Resolve model name (ensures vendor/ prefix)
    resolved_model = resolve_model(model)
    vendor = get_vendor_from_model(resolved_model)

    # Log vendor info (once per vendor per session)
    log_once(
        "info",
    ...
    # Validate resolution if size is specified
    if size:
        try:
            validate_resolution(size, resolved_model)
            logger.debug(f"Resolution '{size}' validated successfully")
        except ValueError as e:
            logger.error(f"Resolution validation failed: {e}")
            raise

    # Gemini uses conversational API (acompletion), others use aimage_generation
    if vendor == "gemini":
        logger.debug("Using gemini conversational API (acompletion)")
        params = {
        ...
        try:
            ...
            logger.info(f"Image generation successful (gemini)")
            return response
        except Exception as e:
            logger.error(f"Image generation failed: {type(e).__name__}: {e}")
            raise
    else:
        logger.debug("Using standard image generation API (aimage_generation)")
...
def list_available_models() -> list[str]:
    """
    List all available models based on current API key configuration.

    Returns:
        List of friendly model names that can be used
    """
    logger.debug("Listing available models based on API keys")
    available = get_available_api_keys()
    models = []
    ...
    if available["gemini"]:
        ...
        logger.debug(f"Added Gemini models: {gemini_models}")
    ...
    if available["openai"]:
        ...
        logger.debug(f"Added OpenAI models: {openai_models}")

    if available["vertex"]:
        models.append("vertex-imagen")
        logger.debug("Added Vertex AI model: vertex-imagen")

    if available["bedrock"]:
        models.append("sdxl")
        logger.debug("Added Bedrock model: sdxl")

    if available["recraft"]:
        models.append("recraft")
        logger.debug("Added Recraft model: recraft")

    logger.debug(f"Total available models: {len(models)}")
    return models
...
async def main():
    ...
    result = await generate_image(
        ...
        # model=None,  # Auto-selects: nano-banana or gpt-image-1
    )
    print(f"Generated image URL: {result.data[0].url}")

    # Explicit model using friendly name
    result = await generate_image(
        prompt="A nano banana dish",
        model="nano-banana",  # Resolves to: gemini/gemini-2.5-flash-image
        n=1,
    )
    ...
    result = await generate_image(
        prompt="A robot",
        model="gpt-1",  # Resolves to: openai/gpt-image-1
        size="1024x1024",
    )

    # Direct litellm format also works
    result = await generate_image(
        prompt="A spaceship",
        model="openai/dall-e-3",  # Already has vendor/ prefix
    )
    ...
    print("Available APIs:", get_available_api_keys())
